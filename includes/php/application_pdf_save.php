<?php
  ...
  include($includes_path.'/php/oaf_vars.php');         //oaf vars contains set session
 ...
 class MYPDF extends TCPDF {
	...
	public function Header() {
		...
		$this->Cell(0, 15, 'Administrative Block, Near bus stand, Mandi, Himachal Pradesh, India', 0, false, 'C', 0, '', 0, false, 'M', 'M');
...
EOD;
	   ...
	   {		
		...
		{
			$degree_array_1[$i] = $row['Degree_Examination'];
			$specialization_array_1[$i] = $row['Specialization'];
			$year_array_1[$i] = $row['Completion_Year'];
			$institute_array_1[$i] = $row['University_Insti'];
			$grade_array_1[$i] = $row['Grade'];
			$i++;
		}

		for (; (list($key_degree, $degree) = each($degree_array_1)) && (list($key_specialization, $specialization) = each($specialization_array_1))
			   && (list($key_year, $year) = each($year_array_1)) && (list($key_institute, $institute) = each($institute_array_1)) 
			   && (list($key_grade, $grade) = each($grade_array_1)) ;) 
		{
			$acad_tbl_1 .= <<<EOD
			<tr>
...
EOD;
		...
		$acad_tbl_1 .='</table>';
        $pdf->writeHTML($acad_tbl_1, false, false, false, false);

	   }
	   else 
			$pdf->MultiCell($width_values, 4, 'No records for Academic Qualifications.', 0, 'L', 0, 1, '' ,'', true);
$pdf->Ln(12);
...
//Font for heading
$pdf->SetFont('times', '', 15);
$pdf->MultiCell($width_heading, 4, 'Teaching Experience', 0, 'C', 0, 1, '' ,'', true);
$pdf->Ln(3);
//Font for details
$pdf->SetFont('times', '', 11);
...
EOD;
		...
		$query = "SELECT * FROM Sponsored_Research_Projects ".
...
$pdf->SetFont('times', '', 11);
	...
	$query = "SELECT Completed, In_Progress, List_Uploaded FROM Thesis_Guidance WHERE User_ID = $user_id ";
		$result = mysqli_query($dbcon_perm, $query) ;
		if(mysqli_num_rows($result)>=1)
	   {		
...
EOD;
		...
		$result = mysqli_query($dbcon_perm, $query);
		$i = 0;
		$row = mysqli_fetch_assoc($result) or die('Unable to fetch data.(Research Papers)');
		...
		$papers_list_uploaded = $row['Publications_List'];
...
$pdf->SetFont('times', '', 12);
	$query = "SELECT Num_Patents, Patents FROM Patents WHERE User_ID = '$user_id'";
...
$pdf->setX(20);
$pdf->SetFont('times', '', 12);
$declaration = "I hereby declare that the entries in this form are true to the best of my knowledge and belief. If at any time I am found to have concealed any material/ information or given any false details, my appointment shall be liable to be summarily terminated without notice or compensation.I hereby declare that the entries in this form are true to the best of my knowledge and belief. If at any time I am found to have concealed any material/ information or given any false details, my appointment shall be liable to be summarily terminated without notice or compensation.";
...
if($papers_list_uploaded)
{
$uploads .=" $counter. List of research papers published/presented.<br/>";
$counter++;
}
...
$pdf->Output($uploaddir.'/Appplication Document.pdf', 'F');
