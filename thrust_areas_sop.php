   ...
   if (isset($_POST['submit']))
   {
		$thrust_area = $_POST['Thrust_Areas'];
	   	$sop = mysqli_real_escape_string($dbcon, trim($_POST['SOP']));
			...
			{
				$query = "UPDATE Thrust_Areas_SOP SET Thrust_Area = '$thrust_area', SOP = '$sop' WHERE User_ID = '" . $_SESSION['user_id'] . "'"; 
				mysqli_query($dbcon, $query);

				$query = "UPDATE Forms_Submitted SET Thrust_Areas_SOP = '1' WHERE User_ID = '" . $_SESSION['user_id'] . "'"; 
				mysqli_query($dbcon, $query);

				//naviate to Undergraduate academic profile
				$next_url = 'http://' . $_SERVER['HTTP_HOST'] . dirname($_SERVER['PHP_SELF']) . '/patents.php';
...
// End of check for form submission
	...
	{
		$query = "SELECT User_ID FROM Thrust_Areas_SOP WHERE User_ID = '" . $_SESSION['user_id'] . "'";
		$data = mysqli_query($dbcon, $query);
		if (mysqli_num_rows($data) == 1) ;
		else 
		{
			$query = "INSERT INTO Thrust_Areas_SOP (User_ID) VALUES ('" . $_SESSION['user_id'] . "')";
			mysqli_query($dbcon, $query)
			or die('Error storing data!');
		}
		// Grab the profile data from the database
		$query = "SELECT * FROM Thrust_Areas_SOP WHERE User_ID = '" . $_SESSION['user_id'] . "'";
		$data = mysqli_query($dbcon, $query);
...
<?php  echo '<p class="error_text">'.$error_msg.'</p>';  ?>
 ...
 <table>
<tr>
		 <td class="labelcell1"><label for="thrust_areas">Thrust Areas:</label></td>
</tr>
<tr>
...
<tr><td></td></tr>
<tr>
	<td class="button"><input type="submit" name="submit" value="Save and Proceed" size = "15"
...
<br>
<div id="navbuttons">
<input type="button" value="Back" size = "15" onclick="parent.location= 'best_papers.php' ">
<input type="button" value="Skip this Page" size = "15" onclick="parent.location= 'patents.php' ">
</div>
